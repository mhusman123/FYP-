...
export async function POST(req: NextRequest) {
  try {
    ...
    // Simulate AI autograding process with logging
    ...
    if (result.error) {
      return NextResponse.json(
        { error: 'Autograding failed', message: result.error },
        { status: 500 }
      );
    }

    console.log(`[AI Autograder] Successfully graded submission ${submission.id} with score ${result.data!.score}`);
